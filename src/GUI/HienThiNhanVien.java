...
public class HienThiNhanVien extends FormHienThi {
    ...
    BUS.NhanVien_BUS qlnv = new BUS.NhanVien_BUS();
    ...
    public HienThiNhanVien() {
        ...
        mtb.setHeaders(new String[]{"STT", "Mã nhân viên", "Tên nhân viên", "Giới tính", "Ngày sinh", "Số điện thoại", "CMND", "Chức vụ", "Ngày vào làm"});
        mtb.setColumnsWidth(new double[]{0.5, 1.5, 0.5, 3, 1.5, 1.5, 1.5, 3, 1.5});
        mtb.setAlignment(0, JLabel.CENTER);
        mtb.setupSort();
        setDataToTable(qlnv.getArrayList_normal(), mtb);

        // ======== search panel ===========
        cbTypeSearch = new JComboBox<>(new String[]{"Mã nhân viên", "Tên nhân viên", "Số điện thoại", "CMND"});
    ...
    private void txSearchOnChange() {
        ...
        String sDate = "";
        String eDate = "";
        if(ngay1!=null) sDate = ngay1.toString();
        if(ngay2!=null) eDate = ngay1.toString();
        setDataToTable(qlnv.search(txTim.getText(), cbTypeSearch.getSelectedItem().toString(), sDate, eDate), mtb);
    }

    private void setDataToTable(ArrayList<NhanVien> data, MyTable table) {
        table.clear();
        int stt = 1; // lưu số thứ tự dòng hiện tại
        for (NhanVien employee : data) {
            ...
            String ngayvaolam  = employee.getNgayvaolam();
                table.addRow(new String[]{
                    String.valueOf(stt), maNV, tenNV, birthDate, gender, sdt, CMND, chucvu, ngayvaolam
                });
                stt++;            
        }
    }
